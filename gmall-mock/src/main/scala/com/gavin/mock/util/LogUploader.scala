...
object LogUploader {
  ...
  def sendLog(log: String): Unit = {
    try {
      // 1. 日志服务器的地址
      //val logUrl = new URL("http://localhost:8081/log") //本地
      //val logUrl = new URL("http://hadoop102:8081/log") // 单独hadoop102
      val logUrl = new URL("http://hadoop102:80/log") // 配置了nginx负载均衡后
      ...
      // 200 => 成功, 404 => Not Found, 500 服务器问题
